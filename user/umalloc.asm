...
free(void *ap)
{
  ...
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
   ...
   e:	7de7b783          	ld	a5,2014(a5) # 7e8 <freep>
  ...
  3c:	00e6ea63          	bltu	a3,a4,50 <free+0x50>
{
  ...
  7e:	76f73723          	sd	a5,1902(a4) # 7e8 <freep>
}
  82:	6422                	ld	s0,8(sp)
...
malloc(uint nbytes)
{
  ...
  b0:	73c53503          	ld	a0,1852(a0) # 7e8 <freep>
  ...
  d8:	71490913          	addi	s2,s2,1812 # 7e8 <freep>
  if(p == (char*)-1)
  dc:	5afd                	li	s5,-1
  de:	a88d                	j	150 <malloc+0xc8>
    base.s.ptr = freep = prevp = &base;
  e0:	00000797          	auipc	a5,0x0
  e4:	71078793          	addi	a5,a5,1808 # 7f0 <base>
  e8:	00000717          	auipc	a4,0x0
  ec:	70f73023          	sd	a5,1792(a4) # 7e8 <freep>
  f0:	e39c                	sd	a5,0(a5)
    base.s.size = 0;
 ...
 112:	6ca73d23          	sd	a0,1754(a4) # 7e8 <freep>
      return (void*)(p + 1);
...
atoi(const char *s)
{
 ...
 30e:	fd07851b          	addiw	a0,a5,-48
  while('0' <= *s && *s <= '9')
 312:	0006c603          	lbu	a2,0(a3) # 1000 <__global_pointer$+0x1f>
...
000000000000049a <rcu_read_only>:
...
00000000000004a2 <rcu_read_heavy>:
...
00000000000004aa <rcu_read_write_mix>:
...
00000000000004b2 <rcu_read_stress>:
...
putc(int fd, char c)
{
 ...
 4d0:	f36080e7          	jalr	-202(ra) # 402 <write>
}
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 502:	2d250513          	addi	a0,a0,722 # 7d0 <digits>
 506:	883a                	mv	a6,a4
...
vprintf(int fd, const char *fmt, va_list ap)
{
 ...
 5ce:	206b8b93          	addi	s7,s7,518 # 7d0 <digits>
 ...
 70e:	0be90913          	addi	s2,s2,190 # 7c8 <printf+0x36>
        while(*s != 0){
